...
st.title("🔍 Ứng dụng OCR kiểm tra mã RPs từ ảnh")

uploaded_files = st.file_uploader("📁 Chọn một hoặc nhiều ảnh bản vẽ", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True)

if uploaded_files:
    st.success(f"Đã tải lên {len(uploaded_files)} ảnh")
    st.write("💡 Bấm 'Phân tích' để bắt đầu nhận diện mã")
    ...
    if st.button("🚀 Phân tích"):
        ...
        st.download_button("⬇️ Tải file CSV", data=csv, file_name="rp_codes.csv", mime="text/csv")
else:
